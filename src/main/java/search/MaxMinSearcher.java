...
public class MaxMinSearcher implements Searcher {
    ...
    private MaxMinSearchResult search(Chessboard chessboard, int level, ChessTypeEnum searchFor, ChessTypeEnum turnToMove) {
        ...
        if (level == searchLevel) {
            ...
            return new MaxMinSearchResult(placeScore + patternScore, null);
        }
        ...
        if (CollectionUtils.isEmpty(blankPlaces)) {
            return new MaxMinSearchResult(0, null);
        }
        ...
        return new MaxMinSearchResult(score, place);
    }
}
